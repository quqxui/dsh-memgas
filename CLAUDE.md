...
当前阶段：M1 实施中。`packages/core` 有存储、双基线通道、RRF 融合与检索编排；`packages/dsh-plugin` 有三个工具与 bundle 清单。动手前先读 README 的「设计原则」「路线图」「未决问题」三节。

```sh
pnpm test        # 全量测试，当前 65 个
...
- 工具链：Node 26 已装，pnpm 未装（M0 需 `corepack enable` 或 `npm i -g pnpm`）。
